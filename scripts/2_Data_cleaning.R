...
#Calculamos el porcentaje de missing values por variable ---------------------------

missing_percentage <- sapply(base_fin, function(y) sum(length(which(is.na(y))))/length(base_fin$directorio))

#Creamos una función para identificar el porcetnaje de NA´s hay por columna ------------------------

data_x <- as.data.frame(missing_percentage)
View(data_x)

#Eliminamos las variables con un alto porcentaje de missing value (>50%)----------------------
...
conteo_variables #comprobamos el número de variables para saber cuántas deben ir a la db de trabajo
